...
1. **Context Layering**
   ...
   - Add progressive implementation details
   - Reference previous successes
   ```
   Building on our authentication system from Phase 1, 
   where we implemented admin routing (useSession.ts), 
   now we need to extend the agent configuration...
   ```

2. **Pattern Anchoring**
   - Begin with familiar patterns
   - Bridge to custom implementations
   - Maintain consistent mental models
   ```
   Similar to how Express middleware chains requests,
   our agent workflow will chain document processing...
   ```

3. **Capability Framing**
   - Frame tasks within Copilot's strengths
   - Break complex features into digestible units
   - Use familiar technical analogies
   ```
   Think of our agent system like a middleware pipeline,
   where each agent is a pure function transforming data...
   ```
...
## **COPILOT ANALYSIS: STRENGTHS & WEAKNESSES**

### ✅ **COPILOT STRENGTHS:**
- **Excellent at single-file implementations** with clear requirements
- **Strong pattern recognition** for common frameworks (React, Node.js, TypeScript)
- **Good at component-level development** with specific inputs/outputs
- **Reliable for API integrations** with well-documented endpoints
- **Efficient at UI/UX implementation** with detailed mockups
- **Solid testing framework** generation and implementation

### ❌ **COPILOT WEAKNESSES:**
- **Struggles with complex system architecture** spanning multiple files
- **Cannot handle business logic orchestration** across services
- **Limited context retention** between large development sessions
- **Difficulty with abstract concepts** like AI agent coordination
- **Overwhelmed by multi-service integration** planning
- **Cannot design database schemas** for complex relationships
