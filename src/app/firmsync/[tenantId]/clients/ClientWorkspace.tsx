...
export default function ClientWorkspace({ tenantId, previewMode = false }: ClientWorkspaceProps) {
  const isPreview = previewMode;
  const { features, loading: featuresLoading, error: featuresError } = useClientFeatures(tenantId);
  const { clients, loading: clientsLoading, error: clientsError, addClient, updateClient } = useClients(tenantId);
  ...
  const handleContactUpdate = async (updates: Partial<Client>) => {
    if (!currentClient || isPreview) return;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-6">
            <div>
              ...
              {isPreview && (
                <p className="text-sm text-amber-600 mt-2">
                  Preview mode: interactions are read-only.
                </p>
              )}
            </div>
            ...
            <button
              ...
              disabled={isPreview}
              className={`px-4 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium ${
                isPreview
            ...
            {showNewClientForm ? (
              <div className="bg-white rounded-lg border border-gray-200 p-6">
                ...
                <form onSubmit={(e) => {
                  ...
                  const statusValue = formData.get('status') as Client['status'] | null;
                  handleClientSubmit({
                    ...
                    status: statusValue ?? 'prospective',
                  });
                }}>
