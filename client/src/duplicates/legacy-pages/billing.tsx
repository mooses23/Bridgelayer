...
function PaymentProcessing() {
  ...
  const createPaymentIntentMutation = useMutation({
    mutationFn: (data: { invoiceId: number; amount: number }) => 
      apiRequest("POST", "/api/app/billing/create-payment-intent", data),
  ...
  const handleCreatePayment = () => {
    if (selectedInvoice && paymentAmount) {
      createPaymentIntentMutation.mutate({
        ...
        amount: parseFloat(paymentAmount), // Amount in dollars (backend will convert to cents)
      });
    }
  };
...
function AdminSettings() {
  ...
  // Use useEffect to handle settings data
  useEffect(() => {
    if (settings && typeof settings === 'object') {
      setBillingSettings(settings as any);
    }
  }, [settings]);
